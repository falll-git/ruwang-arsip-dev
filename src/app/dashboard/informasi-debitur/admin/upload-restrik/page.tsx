...
export default function UploadRestrikPage() {
  ...
  const [pendingUploadItem, setPendingUploadItem] =
    useState<UploadRestrik | null>(null);
  const [dragActive, setDragActive] = useState(false);
  const [selectedFile, setSelectedFile] = useState<File | null>(null);
  const { showToast } = useAppToast();

  const uploadRestrikFile = useCallback(
    async (file: File): Promise<UploadRestrik> => {
      return {
        id: `RST-${createSafeId("restrik")}`,
        fileName: file.name,
  ...
  const handleUpload = async (file: File) => {
    ...
    try {
      const uploadedItem = await uploadRestrikFile(file);
